...
void AESExpandKey256(uint *keybuf)
{
	...
	#ifdef CRYPTONIGHT_V8
		#define STRIDED_INDEX 2
	#else
...
#if defined(CRYPTONIGHT_HEAVY) || defined(CRYPTONIGHT_HAVEN)
		if (variant >= 3) 
		{
...
#if defined(CRYPTONIGHT_V8) && defined(__NV_CL_C_VERSION)
...
__kernel void search1(__global uint4 *Scratchpad, __global ulong *states, __global int *terminate, const uint variant, __global ulong *input)
{
    //ulong a[2], b[2];
	ulong a[2];
	...
	#if !defined(CRYPTONIGHT_HEAVY) && !defined(CRYPTONIGHT_HAVEN) && !defined(CRYPTONIGHT_V8) 
		if (variant > 6) 
		{ 
    ...
    if (gIdx < get_global_size(0)) 
	{
		...
		for (int i = 0; i < iterations; ++i)
		{
			...
			SCRATCHPAD_CHUNK(0) = ((uint4 *)a)[0];
				...
				long prev = *((__global long*)(Scratchpad + (IDX((c[0]) >> 4))));
				long cur = *((__global long*)(Scratchpad + (IDX((c[0]) >> 4))) + 1);
...
__attribute__((reqd_work_group_size(WORKSIZE, 8, 1)))
__kernel void search2(__global uint4 *Scratchpad, __global ulong *states, const uint variant)
{
    __local uint AES0[256], AES1[256], AES2[256], AES3[256];
